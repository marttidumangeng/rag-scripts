...
def main():
    ...
    # The topic pipeline is unpacked fresh into a scratch dir every run, so a
    # sandbox path is expected there. The daily pipeline keeps persistent
    # state and must live under a project directory.
    if (args.pipeline == "daily" and str(root).startswith("/home/ubuntu")
            and "/projects/" not in str(root)):
        errors.append(
            f"{root} is under the ephemeral sandbox home. Install into the "
            "PROJECT shared-file directory (/home/ubuntu/projects/<project>/"
            "...) or the scan baselines and dedup memory are lost on the "
            "next run.")
