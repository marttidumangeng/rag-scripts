"""Audit XRobotics (114) + Foundation (851) for next US enrich."""
...
TARGETS = {
    114: [5289],  # XRobotics xPizza Cube
    851: [2883],  # Foundation Phantom
}
...
def audit_robot(client: ResearchApiClient, rid: int) -> None:
    r = client._get(f"robots/robots/{rid}/")
    img = r.get("image") or r.get("s3_image") or ""
    print(f"\n=== {rid} {r.get('name')}")
    print(" status", r.get("status"), "company", r.get("company"))
    print(" url", r.get("url"))
    print(" img", img)
    if img:
        try:
    ...
    print(" feats len", len(feats), (feats or "")[:280])
    specs = {
        k: r.get(k)
        for k in (
            "payload_kg",
    ...
    for o in r.get("company_owners") or []:
        if isinstance(o, dict) and o.get("slug"):
            slug = o["slug"]
            print(" company_slug", slug, o.get("id"))
    return


def oem_snip(url: str) -> None:
    try:
...
def main() -> int:
    ...
    for cid, ids in TARGETS.items():
        print(f"\n######## company {cid}")
        for rid in ids:
            audit_robot(client, rid)
    oem_snip("https://www.xrobotics.io/")
    oem_snip("https://www.xrobotics.io/specification")
    oem_snip("https://foundation.bot/")
    oem_snip("https://foundation.bot/phantom")
    return 0
