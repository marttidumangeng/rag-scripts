"""Inspect Carbon Robotics (266) fleet."""
...
hashes = {}
for r in c.list_robots_for_company(266) or []:
    ...
    print("  family", d.get("family_key"), d.get("family_name"), d.get("family_url"))
    print("  videos", len(d.get("videos") or []), "tags", d.get("tags"))
    if img:
        try:
            resp = requests.get(img, timeout=60)
            h = hashlib.md5(resp.content).hexdigest()[:12]
            hashes[d["id"]] = (h, len(resp.content), resp.status_code)
            print("  hash", h, len(resp.content), resp.status_code)
        except Exception as e:
            print("  img err", e)
print("unique hashes", len(set(v[0] for v in hashes.values())), hashes)
