"""Final restamp for SC cobots: country + availability in bulk row so stamps are clean."""
...
def main() -> int:
    ...
    for rid in IDS:
        ...
        # post-import patch country+availability again (import stamp can race)
        client._patch(
            f"robots/robots/{rid}/",
            {
                "manufacturer_countries": [CN],
        ...
        # one more restamp after country is set so missing_manufacturer_country clears
        ...
        ref = r2.get("manufacturer_country_ref")
        print(
            json.dumps(
                {
                    "id": rid,
                    ...
                    "errors": errs,
                    "warns": warns,
                    "country_ref": ref.get("id") if isinstance(ref, dict) else ref,
                    "avail": (r2.get("availability_status") or {}).get("key")
                    if isinstance(r2.get("availability_status"), dict)
                    else r2.get("availability_status"),
                    "purpose_len": len((r2.get("purpose") or "").strip()),
