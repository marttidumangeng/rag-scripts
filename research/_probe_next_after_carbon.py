"""Probe next US OEM after Carbon."""
...
cands = [259, 307, 12, 18, 36, 837, 39, 368, 13, 15, 826, 223, 271, 274, 311, 209, 69]
for cid in cands:
    ...
    if not pending:
        continue
    # Prefer US
    if code and code not in ("US", "USA") and cid not in (36, 18, 259, 307, 12):
        if code not in ("US",):
            pass
    gaps = []
    for r in pending[:5]:
        ...
        if not d.get("release_year"):
            g.append("no_year")
        if g:
            gaps.append((r["id"], (r.get("name") or "")[:42], ",".join(g)))
    print(
        f"--- {cid} {name} done={cid in done} country={code} pending={len(pending)}"
    )
